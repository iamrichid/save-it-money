...
const WalletScreen: React.FC<WalletScreenProps> = ({ transactions, categories, onNavigate, onAddTransaction }) => {
  ...
  return (
    <div className="relative flex flex-col h-full w-full bg-background-light dark:bg-background-dark">
      <header className="px-6 py-6 bg-white dark:bg-surface-dark shadow-sm">
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar px-6 py-6 pb-32">
        {showAdd ? (
          <form onSubmit={handleSubmit} className="bg-white dark:bg-surface-dark p-6 rounded-3xl shadow-lg border border-primary/20 mb-8 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
        ...
        <div className="space-y-4">
          ...
          {transactions.length === 0 ? (
            ...
            transactions.map(t => {
              ...
              return (
                <div key={t.id} className="flex items-center justify-between p-4 bg-white dark:bg-surface-dark rounded-2xl shadow-sm border border-black/5 dark:border-white/5">
